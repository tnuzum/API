...
public class AddStoredBankAccount extends base {
	...
	public void getData() {
		...
		agreementNumbersToUpdateToThisFOP = "A40";
	}
...
//					.log().all()
				...
				.when()
					.body(FinancialPL.getstoredbankaccountsWithoutInvoice(customerId,bankAccountNumber,bankRoutingNumber,accountHolderName,bankAccountType,isBusinessAccount,setAsHouseAccount))
...
//					.log().body()
	...
	@Test (testName="Add Account With Invoice", description="PBI:180170")
	public void addAccountWithInvoice() {
...
//					.log().all()
					...
					.header("Content-Type", "application/json")
				.when()
					.body(FinancialPL.getstoredbankaccountsWithInvoice(customerId,bankAccountNumber,bankRoutingNumber,accountHolderName,bankAccountType,isBusinessAccount,setAsHouseAccount,agreementNumbersToUpdateToThisFOP))
					.post("/api/v3/financial/addstoredbankaccount")
				.then()
//					.log().body()
					.extract().response();
